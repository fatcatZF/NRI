...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        prob = my_softmax(logits, -1) #shape: [batch_size, num_edges, num_edgetypes]
        loss_co = torch.mul(prob[:,:,0].float(), relations_masked.float()).mean() #contrasitive loss
        ...
        co_train.append(loss_co.data[0])
